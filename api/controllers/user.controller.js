...
const sendverificationUser = async (req, res, next) => {
    try {
        const user = await User.findOne({where: {id: req.user.user.id}});

        Email.send(
            user.first_name,
            `${process.env.baseUrl}?token=${user.verification_token}`,
            user.email
        );
        res.status(200).send({status: true, response: "check your email"})
...
const sendforgotPasswordUser = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            Email.sendPasswordForget(
                user.first_name,
                `${process.env.baseUrl_RESET_PASSWORD}?token=${user.token}`,
                user.email
            );
            ...
            console.log(user)
            res.status(400).send({status: false, response: "wrong  email"})
...
module.exports = {
    updateRoleUser,
    sendforgotPasswordUser,
    resetPasswordUser,
    sendverificationUser,
